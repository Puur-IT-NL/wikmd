...
WEB_DEPENDENCIES = {
    ...
    "mermaid.min.js": WebDependency(
        ...
        external="https://cdn.jsdelivr.net/npm/mermaid@11.9.0/dist/mermaid.min.js",
    ),
...
def download_web_deps(logger):
    ...
    for dep_name in WEB_DEPENDENCIES:
        ...
        dep_file_path = path.join(
            path.dirname(__file__), dep.local[1:]
        )  # Drop the first '/' so join works
        ...
        if not path.exists(dep_file_path):
            ...
            if not result.ok:
                raise Exception(
                    f"Error while trying to GET {dep.external} Statuscode: {result.status_code}"
                )
